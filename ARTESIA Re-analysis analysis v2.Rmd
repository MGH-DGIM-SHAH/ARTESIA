...
if (!is.na(ix0)) {
  ...
  if (is.na(pAJ0)) pAJ0 <- tail(comp0$est, 1)
} else pAJ0 <- 0
...
if (!is.na(ix1)) {
  ...
  pAJ1 <- approx(comp1$time, comp1$est, xout = tstar,
                 method = "constant", f = 1, rule = 2, ties = "ordered")$y
  if (is.na(pAJ1)) pAJ1 <- tail(comp1$est, 1)
} else pAJ1 <- 0
...
res_print <- within(res, {
  KM_risk <- pct(KM_risk); AJ_risk <- pct(AJ_risk)
  Diff_AJ_minus_KM <- pct(Diff_AJ_minus_KM)
  CI <- paste0(pct(CI_low), " to ", pct(CI_high))
...
data.frame(
  ...
  Estimate = c(pct(RD_KM_hat), pct(RD_AJ_hat),  pct(Delta_hat)), 
  CI = c(paste0(pct(ci_RD_KM[1]), " to ", pct(ci_RD_KM[2])), 
               paste0(pct(ci_RD_AJ[1]), " to ", pct(ci_RD_AJ[2])),
               paste0(pct(ci_Delta[1]), " to ", pct(ci_Delta[2]))
              )
                      ) %>% gt() %>% 
  cols_label(
...
## format ARR lablels
text_arr_km <- paste0(
  round(RD_KM_hat*-100,2), 
  "% (95% CI, ", 
  round(ci_RD_KM[2]*-100,2),
  " to ",
  round(ci_RD_KM[1]*-100,2), 
  ")"
  )

text_arr_aj <- paste0(
  round(RD_AJ_hat*-100,2), 
  "% (95% CI, ", 
  round(ci_RD_AJ[2]*-100,2),
  " to ",
  round(ci_RD_AJ[1]*-100,2), 
  ")"
  )

text_diff <- paste0(
  round(Delta_hat*100,2), 
  " pp (95% CI, ", 
  round(ci_Delta[1]*100,2),
  " to ",
  round(ci_Delta[2]*100,2), 
  ")"
  )
